...
- **Token-protected HTTP MCP transport.** New `mempalace-mcp-http` console script serves the existing MCP JSON-RPC tool surface over `POST /mcp`, with `GET /healthz` for service checks. Both endpoints require a bearer token from `MEMPALACE_HTTP_TOKEN` or `MEMPALACE_HTTP_TOKEN_FILE` unless unauthenticated local development is explicitly enabled. The HTTP adapter reuses `mempalace.mcp_server.handle_request()` so stdio and HTTP tools stay behaviorally aligned. A `server` extra now installs FastAPI/Uvicorn, and the `gpu` extra installs the CUDA/cuDNN runtime wheels needed for isolated `onnxruntime-gpu` deployments on Linux x86_64. (#TBD)
- **Thin remote client mode for local CLI and stdio MCP.** When `MEMPALACE_HTTP_URL` is configured, `mempalace status`, `mempalace search`, and `mempalace-mcp` forward to the remote HTTP MCP endpoint instead of opening the local palace. Passing `--palace` or setting `MEMPALACE_LOCAL=1` preserves the local escape hatch. This lets low-power client machines use a canonical hosted palace without running local embeddings by default. (#TBD)
- **snow-white-iii systemd deployment templates.** New scripts under `scripts/systemd/` install a dedicated `mempalace` user, stage the app under `/media/u0/OneDrive_Backup/mempalace/app`, create the venv under the same root, bind the HTTP MCP service to the Tailnet address, and enforce `MemoryMax=16G`, `CPUQuota=200%`, `Nice=10`, and low-priority IO. The installer refuses `/media/u0/Extreme SSD`, stages app files without `rsync --delete`, and aborts if the canonical palace path already exists. The ChatGPT staging helper dry-runs by default, mines only the raw `chatgpt` exchange wing unless the legacy heuristic signals pass is explicitly requested, and documents the exocortex/LocalAI bridge as the LLM-classification path. (#TBD)
- **MCP search defaults to candidate-union hybrid retrieval.** `mempalace_search` now defaults to the existing `candidate_strategy="union"` path, merging vector candidates with BM25 candidates before the hybrid re-rank when no strict `max_distance` threshold is supplied. Results expose vector similarity plus `bm25_score`, so hosted clients get semantic retrieval, lexical recall, and re-ranking through the HTTP MCP service. (#TBD)
- **Read-only hosted dashboard.** New `mempalace-dashboard` console script serves a token-protected FastAPI dashboard with packaged static assets for service overview, mining state, taxonomy, hybrid search, drawer lists, and drawer detail. The dashboard runs as a separate service from HTTP MCP, uses distinct browser and upstream MCP bearer tokens, refuses unauthenticated API access, and switches to telemetry-only mode while mining/classification services or matching processes are active. In telemetry-only mode, Chroma-backed search/taxonomy/drawer endpoints return `423 Locked` and do not call MCP heavy tools. The snow-white-iii systemd wrapper and deploy helper bind only to the tailnet, stage only dashboard runtime files, refuse `/media/u0/Extreme SSD`, do not touch existing MemPalace/LocalAI services, and keep LocalAI/checkpoint telemetry bounded and read-only. The docs now include both a public dashboard guide and a local end-user manual for the `snow-white-iii` deployment. (#TBD)
- **ChatGPT signal ontology artifact contract.** Added the durable artifact/progress contract for the planned `chatgpt_signals` ontology recovery pipeline. The contract defines dashboard-readable `progress.json`, append-only phase JSONL, `artifacts_index.json`, accepted/unresolved route records, and routed-copy provenance metadata while preserving immutable source drawers and LocalAI-first verification. (#TBD)
...
- **ChatGPT signal ontology runner ops docs.** Documented the snow-white-iii runner environment for the ontology pipeline: canonical paths under `/media/u0/OneDrive_Backup/mempalace`, LocalAI at `http://snow-white-iii:8080/v1` with token file `/media/u0/OneDrive_Backup/mempalace/secrets/localai_token`, service-user execution, `CPUQuota=200%` and `MemoryMax=16G` for any wrapper, read-only dashboard progress from `/media/u0/OneDrive_Backup/mempalace/data/ontology`, and the safe default of writing `apply_ready_manifest.json` without copying drawers unless an explicit apply flag is used. (#TBD)
- **snow-white-iii staged app update and ontology runner wrappers.** Added operational wrappers for deployments where `/media/u0/OneDrive_Backup/mempalace/app` is a staged app copy rather than a git checkout. The promotion wrapper copies a staged committed tree into the live app without `rsync --delete` and without touching palace data, secrets, or services. The ontology runner wrapper starts the ChatGPT signal ontology pass as the `mempalace` service user in a transient systemd unit with `CPUQuota=200%`, `MemoryMax=16G`, LocalAI-on-LAN defaults, and no drawer-copy materialization unless `--apply-copies` is explicitly passed. (#TBD)
...
- **ChatGPT signal ontology route verifier.** Added a pure `route_verify` helper that builds LocalAI-compatible verification prompts, parses approve/reject decisions for selected-candidate and null routes, preserves route provenance, and emits copy-ready or unresolved verification records without mutating drawers. (#TBD)
- **ChatGPT signal ontology iteration reports.** Added pure iteration decision builders that convert verified routes into append-only accepted/unresolved route records, terminal convergence reports, and apply-ready manifests that exclude stale accepted routes superseded by later unresolved states. (#TBD)
- **Thread-aware ChatGPT signal rebuild groundwork.** Added a deterministic ChatGPT conversation segmenter for the replacement LocalAI signal pass. The segmenter splits long conversations by turn windows, handles oversized first messages without dropping suffix text, preserves stable source/subthread provenance, and provides focused coverage tests for multi-topic conversations before the new `chatgpt_thread_signals` layer is mined. (#TBD)
- **Provenance-rich ChatGPT thread-signal MCP writes.** Added `mempalace_add_signal_drawer`, a deterministic write tool for recovered `chatgpt_thread_signals` drawers. It keys drawer identity from `source_signal_id`, stores flat source/subthread/segment provenance metadata, treats duplicate same-content writes as no-ops, and fails closed on source-signal collisions without mutating existing `chatgpt` or `chatgpt_signals` evidence. (#TBD)
- **Thread-aware LocalAI segment extraction.** Added a new `localai_chatgpt_thread_signals.py` runner that extracts signals per ChatGPT thread segment, writes progressive `progress.json` plus append-only segment extraction/checkpoint/invalid-output artifacts, refuses cloud LLM provider URLs, and supports bounded smoke runs before any drawer materialization. (#TBD)
- **ChatGPT thread-signal reconciliation artifacts.** The thread-aware LocalAI runner now reconciles segment-level extractions into deterministic `reconciled_signals.jsonl` records grouped by conversation source hash and subthread. Reconciliation dedupes overlapping segment items, preserves distinct high-recall signals, carries segment provenance forward, and emits fields ready for the `mempalace_add_signal_drawer` write path. (#TBD)
- **Opt-in ChatGPT thread-signal publisher.** The thread-aware LocalAI runner now has an explicit `--publish` mode that sends reconciled signal records to `mempalace_add_signal_drawer`, writes append-only publish checkpoints, tracks publish progress, treats idempotent no-op writes as success, and keeps metadata fields within the MCP write tool's flat scalar bounds. Publishing is disabled by default. (#TBD)
- **snow-white-iii thread-signal rebuild wrapper.** Added a bounded transient systemd wrapper for the thread-aware ChatGPT signal rebuild on `snow-white-iii`, running as the `mempalace` service user with `CPUQuota=200%`, `MemoryMax=16G`, LocalAI-on-LAN defaults, explicit publish mode, durable artifact paths, configurable publish limits/timeouts, and no cleanup or service restarts. (#TBD)
- **Thread-signal safe resume controls.** `localai_chatgpt_thread_signals.py` now supports `--retry-errors` to re-attempt checkpointed `status: error` segments while still skipping prior `classified` and `invalid_output` rows by default, plus bounded per-segment LocalAI retry attempts via `--provider-max-attempts` / `LOCALAI_THREAD_SIGNAL_PROVIDER_MAX_ATTEMPTS` (default `2`). The `start_chatgpt_thread_signal_rebuild_snow_white_iii.sh` wrapper now forwards these controls with unchanged `CPUQuota=200%`, `MemoryMax=16G`, and no-delete path guards. (#TBD)
- **Pre-LLM ChatGPT archive atlas artifacts.** Added an artifact-only atlas pipeline for large ChatGPT privacy exports, with durable progress, conversation/thread indexes, lexical sketches, local CUDA embedding evidence, conservative topic clusters, and Markdown summary artifacts under `/media/u0/OneDrive_Backup/mempalace/data/chatgpt_archive_atlas/<run_id>/`. The completed full-run reference `atlas_full2_20260512T0412Z_2fc8ac5` materialized `4,283` conversation rows, `4,746` thread rows, `3,120` topic-cluster rows, and `4,746` CUDA embedding rows without LocalAI/cloud calls, MCP publication, or palace drawer mutation. (#TBD)
- **Atlas-guided ChatGPT mining artifact contract.** Added pure schema/builders/validators for the planned `chatgpt_atlas_signals` mining layer, covering candidate bridge records, thread-candidate lookups, candidate coverage, progress, artifact indexes, extraction records, reconciliation records, and publish checkpoints. The contract keeps live extraction no-publish by default, represents noisy atlas clusters without inventing canonical rooms, and allows a future explicit publish gate only for `chatgpt_atlas_signals`. (#TBD)
- **Atlas topic-cluster bridge for guided mining.** Added a pure atlas-guided bridge that validates completed archive-atlas topic clusters and materializes candidate bridge records for downstream LocalAI prompts. The bridge preserves cluster evidence, keeps noisy clusters identity-null, resolves mixed clusters conservatively, and disambiguates duplicate candidate keys deterministically without LocalAI, MCP, Chroma, or palace writes. (#TBD)
- **Atlas thread-candidate coverage for guided mining.** Added a pure coverage helper that maps atlas thread-index rows to candidate bridge hints, materializes thread-candidate lookup records, and emits a coverage report with mapped/mixed/noise/unmapped counts plus duplicate and missing thread-reference diagnostics. (#TBD)
- **Atlas-guided input materialization CLI.** Added `scripts/materialize_chatgpt_atlas_guided_inputs.py` to build `candidate_bridge_records.jsonl`, `atlas_thread_candidates.jsonl`, and `atlas_candidate_coverage.json` directly from archive-atlas `topic_clusters.jsonl` and `thread_index.jsonl`. The CLI uses only local file IO plus the existing pure bridge/coverage helpers, confines live atlas run directories to the canonical OneDrive root, refuses `/media/u0/Extreme SSD`, treats identical existing outputs as already materialized, and fails closed on any non-identical target artifact. (#TBD)
- **Atlas-guided ChatGPT prompt/parser contract.** Added pure prompt and parser helpers for the planned atlas-guided LocalAI extraction mode. Prompts include bounded segment text plus only thread-relevant atlas candidate context; parsed responses emit no-publish extraction records for accepted, null-signal, and invalid-output cases; unknown candidates, canonical mismatches, invalid JSON, bad confidence, and structured write/provider/network control keys fail closed before any runner or palace write path exists. (#TBD)
- **Atlas-guided ChatGPT no-publish extraction mode.** `localai_chatgpt_thread_signals.py` now supports `--atlas-guided`, `--atlas-run-dir`, and `--candidate-records`, consuming atlas bridge/coverage artifacts before LocalAI calls and materializing contract-shaped `progress.json`, `artifacts_index.json`, extraction records, invalid outputs, source/segment checkpoints, copied candidate inputs, and a disabled publish checkpoint. The mode refuses `--publish`, keeps legacy thread-signal behavior unchanged, validates required atlas JSONL inputs strictly before provider calls, defaults implicit atlas runs under `/media/u0/OneDrive_Backup/mempalace/data/atlas_guided_chatgpt_signals/<deterministic-child>/`, requires direct atlas-guided LocalAI URLs to target the canonical `snow-white-iii` LAN endpoint before provider creation, and confines explicit atlas-guided CLI run directories to child directories under the canonical guided run root. (#TBD)
- **Atlas-guided ChatGPT reconciliation and snow-white runner.** Added candidate-aware atlas reconciliation that rewrites `reconciled_signals.jsonl` deterministically from extraction records, dedupes same-candidate overlapping signals, preserves candidate provenance and duplicate provenance, and excludes null/invalid/provider-error rows from accepted reconciliation output. A new `start_chatgpt_atlas_guided_extraction_snow_white_iii.sh` wrapper starts no-publish atlas-guided extraction on `snow-white-iii` as `mempalace` with `CPUQuota=200%`, `MemoryMax=16G`, canonical LocalAI/token/atlas paths, narrow run-directory writes, and static guards against non-snow-white hosts, cloud URLs, publish/MCP args, path traversal, non-canonical roots, and `/media/u0/Extreme SSD`. (#TBD)

### Bug Fixes

- **Dashboard telemetry-only safety now covers thread-signal rebuilds and can be forced.** The dashboard mining detector now recognizes `localai_chatgpt_thread_signals.py` and broader ontology/thread-signal runner process signatures (without relying only on exact transient unit names). Added `MEMPALACE_DASHBOARD_FORCE_TELEMETRY_ONLY=1` to force `423 Locked` on heavy search/taxonomy/drawer endpoints without issuing upstream MCP tool calls, while keeping `/api/overview` and read-only ontology progress/artifact endpoints available during rebuilds. (#TBD)
...
- **Read-only MCP calls no longer create absent palace paths.** `mempalace_status` and taxonomy/list tools previously reached ChromaDB's persistent client even when the configured palace directory did not exist, which could materialize an empty `chroma.sqlite3` on a path the user had not intentionally initialized. The shared MCP collection opener now returns "No palace found" for absent paths unless the caller is on an explicit write/create path. (#TBD)
- **ChatGPT privacy exports no longer collapse all threads into one room.** `conversations.json` and Claude privacy-export lists now preserve transcript boundaries in `normalize()`, and the conversation miner chunks and detects rooms per transcript block while keeping a single purge/upsert pass for the source file. This prevents large exports from filing unrelated threads into one gross room while preserving deterministic drawer IDs and stale-drawer rebuild behavior. (#TBD)
- **Overlapping ChatGPT exports dedupe by conversation identity.** Future `conversations.json` imports now file each ChatGPT conversation under a stable logical source ID (`chatgpt:<conversation_id>` or a mapping digest fallback) and a selected-transcript `source_hash`. Re-importing an overlapping export skips unchanged conversations, while changed conversations replace only their own logical-source drawers. Existing path-keyed raw drawers are grandfathered and left untouched. The LocalAI signal pass now uses the same logical IDs for new checkpoints while honoring classified legacy checkpoint rows. (#TBD)
- **Thread-signal rebuild now survives malformed source files.** The thread-aware LocalAI runner no longer aborts the entire run on a malformed or truncated `conversations.json`; it records durable source-level errors/checkpoints (`source_file_errors.jsonl`, `source_checkpoint.jsonl`), updates progress counters, and continues to remaining source files without mutating source data. (#TBD)
