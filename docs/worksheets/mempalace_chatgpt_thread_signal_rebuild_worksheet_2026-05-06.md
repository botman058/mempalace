...
- Top seed-prompt item counts: `fact: 19,977`, `task: 7,812`, `problem: 6,070`, `decision: 4,663`, `open_question: 3,847`, `preference: 3,254`, and `project: 2,496`.
- Preview evidence shows useful durable signals across concrete tasks, preferences, decisions, problems, projects, factual notes, and evidence snippets.
- Correction: the seed-prompt types above are not the final canonical ontology. The point of this pass is to generate candidate canonical signal types and candidate rooms from the corpus.
- Candidate taxonomy evidence: the model is already emitting useful candidate labels beyond the seed prompt, including `recommendation`, `solution`, `example`, `goal`, `risk`, `strategy`, and `constraint`.
...
- Operational risk: the active fallback unit was started with `--publish`, and `scripts/localai_chatgpt_thread_signals.py` performs reconciliation and publication at the end of the same process. Before WP-10 reaches completion, `O-0` should decide whether this run should publish a candidate-taxonomy layer directly or complete extraction/reconciliation first for review.
- Preview verdict: the extraction direction is broadly correct for candidate canonical type discovery, but final publish should be gated on duplicate reconciliation review, candidate type/room review, and improved inspection of noisy/boilerplate open-question items.
